...
Follow the [Quick start in README.md](README.md#quick-start) to clone, install, configure `.env` and `config/config.json`, and run the server. Development and tests run under Node — see the [Test runner caveat](#test-runner-caveat) below.

## Dev commands

```bash
pnpm dev            # tsx watch src/server/index.ts (Node; hot-restart on change)
pnpm build          # vite build → dist/
pnpm test           # vitest run (under Node)
...
- **TypeScript with `"strict": true`.** New code should pass `pnpm exec tsc --noEmit` cleanly. There is a backlog of pre-existing strict-mode noise — don't add to it.
...
`pnpm test` runs Vitest **under Node** (`vitest run`). Don't switch it to Bun's
native `bun test` — the suite uses Vitest APIs like `vi.hoisted` that Bun's test
runner doesn't support. Development also runs under Node (`tsx watch`), not
`bun --hot`, which leaked grandchild processes on Windows.

SQLite goes through a runtime-detecting shim, [`src/shims/bun-sqlite.ts`](src/shims/bun-sqlite.ts):
`node:sqlite` under Node (tests, dev), `bun:sqlite` under Bun (the Fly production
image). One shim serves every runtime — source imports it instead of `bun:sqlite`
directly. If you add SQLite-touching code, the shim covers the API surface
(`new Database(path)`, `db.exec`, `db.prepare(...).all/.run/.get`); extend it if not.
...
- [ ] `pnpm build` passes.
- [ ] `pnpm test` passes (481+ tests).
- [ ] `pnpm exec vitest run tests/performance.test.ts` passes (no perf regressions).
- [ ] `pnpm exec tsc --noEmit` doesn't introduce new type errors (count the existing ones, don't grow the count).
