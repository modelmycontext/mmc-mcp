...
import { readConfig, resolveConfigPath } from '@src/admin/configStore.js';
...
/**
 * Map a repo path for a form template to its local on-disk layout. Forms are
 * co-located with their model in the org repo: `<activity>/forms/<formId>.json`
 * → `<skillsDir>/<activity>/forms/<formId>.json`. A top-level `forms/<formId>.json`
 * (no activity segment) → `<skillsDir>/forms/<formId>.json`. Returns null if the
 * path is not a form template (a `.json` inside a `forms/` directory). Pure —
 * the single source of truth for the forms layout, shared by save and delete.
 */
export function resolveFormLocalPath(
  skillsDir: string,
  remotePath: string,
): { localPath: string; relPath: string } | null {
  const parts = remotePath.split('/');
  const fileName = parts[parts.length - 1];
  if (!fileName.endsWith('.json') || !parts.includes('forms')) return null;
  const formsIdx = parts.indexOf('forms');
  const activityName = formsIdx > 0 ? parts[formsIdx - 1] : '';
  const localPath = activityName
    ? path.join(skillsDir, activityName, 'forms', fileName)
    : path.join(skillsDir, 'forms', fileName);
  const relPath = activityName ? `${activityName}/forms/${fileName}` : `forms/${fileName}`;
...
export class SkillSync {
      ...
      if (parsed && typeof parsed === 'object') {
        if (parsed.content) {
          if (parsed.encoding === 'base64') {
             // GitHub's REST API returns base64 LINE-WRAPPED with `\n` every 60
             // chars. The old guard ("decoded if it contains `\n` or starts with
             // `#`") therefore misfired on every wrapped payload and wrote raw
             // base64 to disk — breaking readModel() (plain JSON.parse) for every
             // model and skill. Trust the `encoding` field instead: strip ALL
             // whitespace and decode iff the result is valid base64. Content that
             // an upstream layer already decoded is not pure base64 (markdown has
             // `#`, spaces, `:`), so the regex fails and we return it untouched.
             const cleanBase64 = parsed.content.replace(/\s/g, '');
             if (/^[A-Za-z0-9+/]*={0,2}$/.test(cleanBase64) && cleanBase64.length > 0) {
  ...
  /**
   * Fetch a repo path's contents from the GitHub REST API. Returns the response
   * as a JSON string (mirroring what the old GitHub MCP server put in its text
   * field) so the existing parse/decode path is unchanged: a directory yields a
   * JSON array of items, a file yields `{ content, encoding, sha, … }` which
   * decodeGithubContent base64-decodes. Returns null for 404 (absent path).
   *
   * Replaces the deprecated `@modelcontextprotocol/server-github` stdio child —
   * the sync only needs read-only contents, so a direct REST call removes the
   * spawned-process dependency entirely (no bun/npx/Docker; same in dev & prod).
   */
  private async githubGetContents(owner: string, repo: string, repoPath: string, branch?: string): Promise<string | null> {
    const token = process.env.GITHUB_PERSONAL_ACCESS_TOKEN;
    const segments = repoPath.split('/').filter(Boolean).map(encodeURIComponent).join('/');
    const ref = branch ? `?ref=${encodeURIComponent(branch)}` : '';
    const url = `https://api.github.com/repos/${owner}/${repo}/contents/${segments}${ref}`;
    const res = await fetch(url, {
      headers: {
        Accept: 'application/vnd.github+json',
        'User-Agent': 'mmc-mcp-skillsync',
    ...
    if (!res.ok) {
      ...
      throw new Error(`GitHub contents ${res.status} for "${repoPath}": ${body.slice(0, 200)}`);
    }
  ...
  async syncFromGithub(): Promise<string[]> {
    ...
    if (!process.env.GITHUB_PERSONAL_ACCESS_TOKEN) {
      logger.warn('[SkillSync] GITHUB_PERSONAL_ACCESS_TOKEN not set — skipping GitHub sync');
      return syncedFiles;
    }
    ...
    try {
      ...
      logger.error({ configPath: resolveConfigPath(projectRoot), error: e.message }, '[SkillSync] Could not read config.json — skipping sync');
      return syncedFiles;
    }
    ...
    for (const repoInfo of config) {
      ...
      try {
        const repoSynced = await this.syncDirectoryRecursively(owner, repo, repoPath || '', branch);
  ...
  private async deleteLocalFile(remotePath: string): Promise<void> {
    ...
    } else if (remotePath.endsWith('.json') && remotePath.split('/').includes('forms')) {
      // Form template — same layout mapping as saveFormFile.
      localPath = resolveFormLocalPath(this.skillsDir, remotePath)?.localPath ?? null;
  ...
  /**
   * Saves a form-template JSON, co-located with its model. Repo path
   * `<activity>/forms/<formId>.json` → `<skillsDir>/<activity>/forms/<formId>.json`.
   * A top-level repo `forms/<formId>.json` (no activity segment) → `<skillsDir>/forms/<formId>.json`.
   * The public form runner (src/ui/publicForm.ts) reads these.
   */
  private async saveFormFile(filePath: string, content: string): Promise<string | null> {
    const mapped = resolveFormLocalPath(this.skillsDir, filePath);
    if (!mapped) return null;
    await fs.mkdir(path.dirname(mapped.localPath), { recursive: true });
    await fs.writeFile(mapped.localPath, this.decodeGithubContent(content));
  ...
  private async syncDirectoryRecursively(owner: string, repo: string, currentPath: string, branch?: string): Promise<string[]> {
    const syncedFiles: string[] = [];
    let items: any[] | undefined;

    try {
      const textContent = await this.githubGetContents(owner, repo, currentPath, branch);
      if (textContent) {
        try {
    ...
    if (items && Array.isArray(items)) {
      ...
      const results = await Promise.all(items.map(async (item): Promise<string[]> => {
        if (item.type === 'file' && item.name.endsWith('.md')) {
          ...
          try {
            const textContent = await this.githubGetContents(owner, repo, item.path, branch);
            if (textContent) {
        ...
        } else if (item.type === 'file' && item.name.endsWith('.json') && item.path.split('/').includes('forms')) {
          // Form template — a `.json` inside a `forms/` directory. Checked
          // before the model branch because forms also end in `.json`.
          this.remotePathsSeen.add(item.path);
          if (item.sha && this.shaState[item.path] === item.sha) {
            this.counters.skipped++;
            logger.debug({ path: item.path, sha: item.sha }, '[SkillSync] Skipped unchanged form');
            return [];
          }
          try {
            const textContent = await this.githubGetContents(owner, repo, item.path, branch);
            if (textContent) {
              const savedPath = await this.saveFormFile(item.path, textContent);
              if (savedPath) {
                if (item.sha) this.shaState[item.path] = item.sha;
                this.counters.downloaded++;
                logger.info({ path: item.path, sha: item.sha, localPath: savedPath }, '[SkillSync] Downloaded form');
                return [savedPath];
              } else {
                this.counters.writeErrors++;
                logger.warn({ path: item.path }, '[SkillSync] saveFormFile returned null — content not written');
              }
            }
          } catch (e: any) {
            this.counters.fetchErrors++;
            logger.warn({ path: item.path, error: e.message }, '[SkillSync] Form download failed');
          }
          ...
          return this.syncDirectoryRecursively(owner, repo, item.path, branch);
        }
        return [];
      }));
