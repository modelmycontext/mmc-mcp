...
export async function executeViewSlice(
  ...
  for (const query of queries) {
    if (query.job) {
      try {
        const params = resolveJobParams(query.job as AutomationJob, factIdToName, allFactValues);
        const jobResult = await deps.executeConnector(query.job.toolId, params, sessionId);
        applyJobResultToFacts(jobResult, query.job as AutomationJob, allFactValues, recordToolError, 'query', sliceName);
      } catch (err: any) {
