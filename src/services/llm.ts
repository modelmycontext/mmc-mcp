...
// single LLM rule alone exceed the netlify-dev 30s lambda cap on
// `/api/mcp/rpc`. 8s leaves headroom for JSON parse + return inside the
// 25s `get-next-event` cap, while staying generous enough that typical
// completions (~1–4s) aren't cut.
