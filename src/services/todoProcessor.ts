  ...
  async handleEvent(event: Event): Promise<void> {
    ...
    try {
      ...
      for (const slice of workflow.slices) {
        if (!slice.isInterface) continue; // automated slices handled by AutomatedSliceRunner
        ...
        // Check if a todo already exists for this slice+session
        const existing = this.deps.todoStore.findBySliceAndSession(slice.name, sessionId);
        if (existing) {
          // Mark completed if outcome events are on the bus — but only for claimed
          // todos (ones that have actually been executed). Pending todos should remain
          // pending so they can be claimed and displayed to the user, even if outcome
          // events arrive from the cascade before the user picks them up.
          if (existing.status === 'claimed' && slice.outcomeEventTypes.length > 0) {
        ...
        logger.info(
          { sliceName: slice.name, role: slice.role, sessionId, todoId, triggerEventType: event.type },
          '[TodoProcessor] Created pending todo for interface slice'
        );
      }
    } catch (err: any) {
      logger.error(
