 ...
 */
function applyJobResultToFacts(
  jobResult: any,
  job: AutomationJob,
  ...
  return async (event: any) => {
    ...
    try {
      ...
      const applyInstructionResult = (
        ...
        opts: { tool: string; phase: 'query' | 'command'; name: string },
      ) => {
          ...
          if (query.job) {
            ...
            try {
              ...
              const result = await deps.llmService.evaluateInstruction(
                query.text,
                scopeFactsToContract(allFactValues),
                returnedFactName,
                inScopeFactNames,
              );
              applyInstructionResult(result, { tool: 'ai.eval', phase: 'query', name: query.name });
              logger.info(
      ...
      try {
        const commandJob = extractSliceCommandJob(data.slice, factIdToName);
        if (commandJob) {
          commandJobForError = { toolId: commandJob.toolId, name: commandJob.name };
          const params = resolveJobParams(commandJob, factIdToName, allFactValues);
          const jobResult = await deps.executeConnector(commandJob.toolId, params, sessionId);
          applyJobResultToFacts(jobResult, commandJob, allFactValues, recordToolError, 'command', sliceName);
          logger.info(
            { sliceName, job: commandJob.name, outputFields: Object.keys(commandJob.resolvedOutputMappings), legacyReturnedFact: Object.keys(commandJob.resolvedOutputMappings).length === 0 ? commandJob.returnedFact.name : undefined, ms: Date.now() - t25b },
            '[AutomatedSlice] Step 2.5b: command job executed'
          );
          ...
          if (cmdInstruction) {
            ...
            const result = await deps.llmService.evaluateInstruction(
              ...
              scopeFactsToContract(allFactValues),
              cmdInstruction.outputFactName,
              inScopeFactNames,
            );
            applyInstructionResult(result, { tool: 'ai.eval', phase: 'command', name: 'command-instruction' });
            logger.info(
              { sliceName, instruction: cmdInstruction.instruction.slice(0, 80), outputFact: cmdInstruction.outputFactName, resultKeys: Object.keys(result), ms: Date.now() - t25b },
        ...
        logger.warn(
          ...
          '[AutomatedSlice] Step 2.5b: command failed — continuing without command result'
        );
      }
      ...
      const t3 = Date.now();
      const matchingScenarios: typeof eligibleScenarios = [];
      for (const s of eligibleScenarios) {
        const ts = Date.now();
        const givenOk = await evaluateBusinessRules(s.givenBusinessRules, allFactValues, s.factIdToName, llmEvaluator, s.givenBusinessRuleLogic);
        const whenOk = await evaluateBusinessRules(s.whenBusinessRules, allFactValues, s.factIdToName, llmEvaluator, s.whenBusinessRuleLogic);
      ...
      const stalled = matchingScenarios.length === 0 && eligibleScenarios.length > 0;
      if (stalled) {
