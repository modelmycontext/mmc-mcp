...
import { resolvePath, resolveTemplate } from '@src/utils/logicUtils.js';
...
interface BusinessRule {
  ...
  operator: string;
  value?: string;
  compareToFactId?: string;
  compareToFactField?: string;
...
function evaluateRule(rule: BusinessRule, facts: Record<string, any>, factLookup: Map<string, string>): boolean {
  ...
  switch (rule.operator) {
    ...
    default:
      logger.warn({ operator: rule.operator, factName }, `[sliceEvaluator] Unknown operator: ${rule.operator}`);
      return false;
  }
}
  ...
  for (const query of queries) {
    ...
    if (!query.job?.toolId && query.text && llmService) {
      try {
        ...
        const result = await llmService.evaluateInstruction(query.text, enrichedFacts, returnedFactArg);
...
async function runSliceCommand(
  ...
  if (effectiveMode === 'job' && commandJob?.toolId) {
    ...
    // Pass ALL output fact names so the LLM produces one key per fact
    // instead of cramming everything into the first one.
    const outputFactNames = [
      ...((sliceData.outcomes?.[0]?.facts ?? []).map(f => f.name).filter(Boolean) as string[]),
      ...((sliceData.facts ?? []).map(f => f.name).filter(Boolean) as string[]),
    ];
    const uniqueOutputFacts = Array.from(new Set(outputFactNames));
    try {
...
export async function evaluateSlice(
  ...
  const sliceId = `slice-${sliceData.index + 1}-${toKebabCase(sliceData.name)}`;
