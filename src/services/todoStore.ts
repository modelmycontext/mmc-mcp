...
export interface TodoRecord {
  id: string;
  /** Workflow-instance id (workflow-instance-isolation RFC). Was `correlationId`. */
...
/**
 * Behavioural contract shared by the SQLite-backed {@link TodoStore} and the
 * {@link InMemoryTodoStore}. Test-session isolation is achieved by swapping the
 * *backing store* (in-memory) — never by disabling the mechanism — so test
 * sessions exercise the same todo/completion semantics as production.
...
export class TodoStore implements TodoStoreLike {
  private db: Database;
  ...
  private init() {
    ...
    // from `workflowSessionId` → `correlationId`, no backfill. Drop a pre-cutover
    // todos table so the new schema + indexes build cleanly; old todos are
    // abandoned by design (non-production).
    const todoCols = this.db.prepare(`SELECT name FROM pragma_table_info('todos')`).all() as { name: string }[];
    if (todoCols.length > 0 && !todoCols.some(c => c.name === 'correlationId')) {
      this.db.exec(`DROP TABLE todos`);
      logger.warn('[TodoStore] Pre-cutover schema detected — dropped todos table (correlationId hard cutover)');
    }
    this.db.exec(`
    ...
    this.db.exec(`CREATE INDEX IF NOT EXISTS idx_todos_correlation ON todos(correlationId)`);
    this.db.exec(`CREATE INDEX IF NOT EXISTS idx_todos_slice_correlation ON todos(sliceName, correlationId)`);
    logger.info('[TodoStore] Table initialized');
  }

  upsert(todo: TodoRecord): void {
    this.db.prepare(`
      INSERT INTO todos (id, correlationId, sliceName, role, status, triggerEventType, payload, createdAt, claimedBy, claimedAt, completedAt, pattern)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
   ...
   * already exists for the same (sliceName, correlationId). The check
   * and insert happen in a single SQL statement so concurrent event-bus
   * subscribers can't race past a non-atomic `findBySliceAndCorrelation` +
  ...
  insertPendingIfAbsent(todo: TodoRecord): boolean {
    const result = this.db.prepare(`
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM todos
        WHERE sliceName = ? AND correlationId = ? AND status IN ('pending', 'claimed')
      )
    `).run(
      todo.id,
      todo.correlationId,
  ...
  getByCorrelation(correlationId: string): TodoRecord[] {
    const rows = this.db.prepare(
      `SELECT * FROM todos WHERE correlationId = ? ORDER BY createdAt ASC`
    ).all(correlationId);
    return rows.map(this.rowToRecord);
  }
  ...
  findBySliceAndCorrelation(sliceName: string, correlationId: string): TodoRecord | null {
    const row = this.db.prepare(
      `SELECT * FROM todos WHERE sliceName = ? AND correlationId = ? LIMIT 1`
    ).get(sliceName, correlationId) as any;
    return row ? this.rowToRecord(row) : null;
  }
