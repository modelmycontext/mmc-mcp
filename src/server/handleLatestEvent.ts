...
export function dispatchLatestEvent(evt: any, deps: HandleLatestEventDeps): DispatchResult {
  ...
  if (type === 'awaiting_callback') {
    const awaiting: string[] = Array.isArray(evt?.payload?.awaiting) ? evt.payload.awaiting : [];
    const what = awaiting.length ? `\`${awaiting.join('`, `')}\`` : 'an external callback';
    return {
      content: [{ type: 'text', text:
        `The workflow is paused waiting for ${what} from an outside system (e.g. the applicant submitting the form). This is NOT completion — tell the user the run is awaiting the external step, then KEEP polling: call get-next-event again to wait for the callback to arrive (it may take a while).`
      }],
    };
  }
