...
export class ConnectorExecutor {
  private capabilities: Connector[];

  constructor(capabilities: Connector[] = connectors) {
  ...
  createExecutor(action: string, params: any, isSystemStep: boolean = false) {
    return async (ctx: ConnectorContext, input: any) => {
      ...
      // Check for executable capability
      const capability = this.capabilities.find(c => c.name === action || c.name.replace(/:/g, '__') === normalizedAction);
      ...
      if (!tool) {
        if (capability) {
          tool = async (p, i) => {
            return capability.execute(ctx, p, i);
          };
          ...
          tool = async (p, i) => {
            ...
            const { scenarios, template, mappings } = p;
