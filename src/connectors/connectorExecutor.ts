...
export class ConnectorExecutor {
  ...
  createExecutor(action: string, params: any, isSystemStep: boolean = false) {
    return async (ctx: ConnectorContext, input: any) => {
      ...
      if (!isSystemStep) {
        // #76: TOOL_CALLED is a persisted audit/diagnostic record — the
        // event-delivery router drops it from client delivery, so its payload
        // exists only to be written to events.db and surfaced via events-dump.
        // Storing raw params/input/result wrote connector PII and secrets
        // (email bodies, recipient lists, upstream API responses) into the
        // event log at rest, and polluted the session fact pool because
        // getSessionFactValues flattens every payload. Persist metadata only —
        // the tool name plus the SHAPE (top-level key lists) — which keeps the
        // audit trail and eventFormatter output useful without retaining
        // sensitive values. Full values remain in the live server logs for
        // deep debugging.
        const topLevelKeys = (v: unknown): string[] =>
        ...
        await ctx.eventBus.publish({
          ...
          payload: {
            tool: action,
            paramKeys: topLevelKeys(resolvedParams),
            inputKeys: topLevelKeys(input),
            resultKeys: topLevelKeys(finalResult),
          },
          timestamp: new Date(),
