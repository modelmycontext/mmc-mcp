...
pnpm start              # Run the server under Bun (HTTP on :3001 + stdio)
pnpm start:force-sync   # Run under Bun with --force-sync flag
pnpm dev                # Bun --hot dev server
pnpm test               # Run Vitest under Bun (bun x vitest run)
pnpm test:watch         # Run Vitest in watch mode
pnpm debug              # Run under Bun with --inspect
pnpm debug:brk          # Run under Bun with --inspect-brk (pauses on start)
```

Run a single test file:
```bash
bun x vitest run tests/events/eventBus.test.ts
```

> Vitest runs the suite under Node, but production runs on Bun. The `bun:sqlite`
> import is aliased to a Node `node:sqlite`-backed shim at test time
> (see `vitest.config.ts` and `tests/_shims/bun-sqlite.ts`).
...
compatibility: "Requires mmc-mcp tools: json-read, log-event-to-bus"
---
```
...
2. `SkillTools` (`json-read`, `json-write`) — dispatched via `SkillToolExecutor`
