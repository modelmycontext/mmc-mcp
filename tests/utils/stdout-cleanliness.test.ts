...
describe('stdio transport stdout cleanliness', () => {
  it('emits only JSON-RPC frames on stdout during init', async () => {
    // Boot the server under Node via the tsx loader (no Bun). Using
    // process.execPath + `--import tsx` keeps this cross-platform and avoids
    // depending on a `tsx`/`bun` binary being on PATH.
