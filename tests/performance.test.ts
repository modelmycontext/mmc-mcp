...
describe('Performance: SqliteEventStore', () => {
  ...
  it('appends 1 000 events in < 10 000 ms', async () => {
    ...
    for (let i = 0; i < 1_000; i++) {
      await store.append(
        makeEvent({ id: `e-${i}`, sequence: i + 1, sessionId: 'perf-sess' })
      );
    }
    const elapsed = performance.now() - start;
  ...
  it('getNextAfterSequence on 1 000 rows completes in < 100 ms', async () => {
    for (let i = 0; i < 1_000; i++) {
      await store.append(
        makeEvent({ id: `e-${i}`, type: i % 2 === 0 ? 'A' : 'B', sequence: i + 1, sessionId: 'perf-sess' })
      );
    }
  ...
  it('getSessionFactValues with 500 events completes in < 200 ms', async () => {
    for (let i = 0; i < 500; i++) {
      await store.append(
        makeEvent({
          id: `e-${i}`,
    ...
    const start = performance.now();
    const facts = store.getSessionFactValues('perf-sess');
    const elapsed = performance.now() - start;
  ...
  it('getSessionFactValues cache hit is < 5 ms', async () => {
    for (let i = 0; i < 200; i++) {
      await store.append(
        makeEvent({
          id: `e-${i}`,
    ...
    // Prime the cache
    store.getSessionFactValues('cache-sess');

    const start = performance.now();
    const facts = store.getSessionFactValues('cache-sess');
    const elapsed = performance.now() - start;
  ...
  it('cache invalidation on append works correctly', async () => {
    await store.append(
      makeEvent({ id: 'e1', sequence: 1, sessionId: 's1', payload: { tier: 'silver' } })
    );
    // Prime cache
    expect(store.getSessionFactValues('s1').tier).toBe('silver');

    // Append new event — should invalidate
    await store.append(
      makeEvent({ id: 'e2', sequence: 2, sessionId: 's1', payload: { tier: 'gold' } })
    );
    expect(store.getSessionFactValues('s1').tier).toBe('gold');
  });
});
