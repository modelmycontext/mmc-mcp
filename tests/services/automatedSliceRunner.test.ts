...
import { createAutomatedSliceHandler, resolveInlineSliceData, applyJobResultToFacts, type SliceData } from '../../src/services/automatedSliceRunner.js';
...
describe('createAutomatedSliceHandler', () => {
  ...
  describe('Step 1 — eligibility', () => {
    ...
    it('filters out scenarios whose given[] events are not all present on the session', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationEventTypes.mockReturnValue(new Set(['existing-event']));
      const data = makeSliceData({
  ...
  describe('Step 2 — fact collection', () => {
    it('merges session fact values with triggering event payload', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ sessionFact: 'from-session' });
    ...
    it('triggering event payload overrides session fact values for same key', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ tier: 'silver' });
  ...
  describe('Step 3 — business rule evaluation', () => {
    ...
    it('skips scenario when givenBusinessRules fails', async () => {
      ...
      const handler = createAutomatedSliceHandler(data, deps as any);
      await handler(makeTriggerEvent());
      // No OUTCOME may fire — the only allowed publication is the
      // slice-stalled timeline diagnostic the no-match path emits.
      const published = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls.map(c => c[0]);
      expect(published.filter(e => e.type !== 'slice-stalled')).toHaveLength(0);
    });
  ...
  describe('Step 3 — first-match selection (#78)', () => {
    const outcomeTypes = (deps: ReturnType<typeof makeDeps>) =>
      (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls.map(c => c[0].type);

    it('fires only the FIRST matching scenario when several would match (no all-match)', async () => {
      mockEvaluateBusinessRules.mockResolvedValue(true); // every rule passes
      const deps = makeDeps();
      const data = makeSliceData({
        scenarios: [
          makeRawScenario({ id: 'first', then: [{ name: 'outcome-a', facts: [] }] }),
          makeRawScenario({ id: 'second', then: [{ name: 'outcome-b', facts: [] }] }),
        ],
      });
      await createAutomatedSliceHandler(data, deps as any)(makeTriggerEvent());
      expect(outcomeTypes(deps)).toEqual(['outcome-a']);
      expect(outcomeTypes(deps)).not.toContain('outcome-b');
    });

    it('skips an earlier non-matching scenario and fires the first that matches', async () => {
      // First scenario's rules fail, second's pass — selection must reach the
      // second. Empty rule arrays (e.g. givenBusinessRules: []) pass, matching
      // the real evaluator's "no rules = true".
      mockEvaluateBusinessRules.mockImplementation(async (rules: any[]) =>
        Array.isArray(rules) && rules.length > 0 ? rules.some(r => r?.pass === true) : true,
      );
      const deps = makeDeps();
      const data = makeSliceData({
        scenarios: [
          makeRawScenario({ id: 'first', whenBusinessRules: [{ pass: false }], then: [{ name: 'outcome-a', facts: [] }] }),
          makeRawScenario({ id: 'second', whenBusinessRules: [{ pass: true }], then: [{ name: 'outcome-b', facts: [] }] }),
        ],
      });
      await createAutomatedSliceHandler(data, deps as any)(makeTriggerEvent());
      expect(outcomeTypes(deps)).toEqual(['outcome-b']);
    });

    it('a no-rule catch-all placed last acts as the otherwise branch', async () => {
      // Specific scenario fails; trailing no-rule scenario wins.
      mockEvaluateBusinessRules.mockImplementation(async (rules: any[]) =>
        Array.isArray(rules) && rules.length > 0 ? rules.some(r => r?.pass === true) : true,
      );
      const deps = makeDeps();
      const data = makeSliceData({
        scenarios: [
          makeRawScenario({ id: 'specific', whenBusinessRules: [{ pass: false }], then: [{ name: 'specific-outcome', facts: [] }] }),
          makeRawScenario({ id: 'catch-all', whenBusinessRules: [], then: [{ name: 'default-outcome', facts: [] }] }),
        ],
      });
      await createAutomatedSliceHandler(data, deps as any)(makeTriggerEvent());
      expect(outcomeTypes(deps)).toEqual(['default-outcome']);
    });
  });
  ...
  describe('Step 4 — outcome publishing', () => {
    ...
    it('resolves calculatedValue formula for outcome facts', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ orderValue: '500' });
      const data = makeSliceData({
        scenarios: [makeRawScenario({
          then: [{
    ...
    it('uses defaultValue when calculatedValue is absent', async () => {
      ...
      await handler(makeTriggerEvent());
      const payload = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0].payload;
      expect(payload.f1).toBe('standard');
    });

    it('prefers session fact over defaultValue when both exist', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ 'total-amount': '9000' });
      const data = makeSliceData({
        scenarios: [makeRawScenario({
          then: [{
            name: 'activated',
      ...
      await handler(makeTriggerEvent());
      const payload = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0].payload;
      expect(payload.f1).toBe('9000');
    });

    it('uses calculatedValue even when session fact exists', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ approved: 'false' });
      const data = makeSliceData({
        scenarios: [makeRawScenario({
          then: [{
            name: 'approved-event',
      ...
      await handler(makeTriggerEvent());
      const payload = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0].payload;
      expect(payload.f1).toBe('true');
    });

    it('copies current fact value when both calculatedValue and defaultValue are empty', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ tier: 'gold' });
      const data = makeSliceData({
        scenarios: [makeRawScenario({
          then: [{
            name: 'tier-confirmed',
    ...
    it('publishes slice-tool-failed when no scenario matches and _tool_errors are present', async () => {
      ...
      expect(stallEvent.correlationId).toBe('sess-1');
    });

    it('publishes slice-stalled (not slice-tool-failed) when stall has no _tool_errors', async () => {
      // A stall with no upstream tool failure used to be a server-log-only WARN.
      // That silence cost real debugging time (a run "completes" with a branch
      // missing and the panel shows nothing), so the diagnostic now publishes
      // as a `slice-stalled` session event the test panel renders as a red
      // card. slice-tool-failed remains reserved for genuine tool errors.
      mockEvaluateBusinessRules.mockResolvedValue(false);
      const deps = makeDeps();
      const data = makeSliceData({
        scenarios: [makeRawScenario({
      ...
      const stalledEvent = published.find(e => e.type === 'slice-stalled');
      expect(stalledEvent).toBeDefined();
      expect(stalledEvent.correlationId).toBe('sess-1');
      expect(stalledEvent.payload.sliceName).toBe('my-activity/my-slice');
      expect(['undefined-facts', 'rule-logic-miss']).toContain(stalledEvent.payload.reason);
      expect(stalledEvent.payload.errors).toHaveLength(1);
      expect(stalledEvent.payload.errors[0].error).toMatch(/No scenarios matched/);
      expect(Array.isArray(stalledEvent.payload.scenarios)).toBe(true);
      expect(stalledEvent.payload.scenarios[0].rules.length).toBeGreaterThan(0);
    });

    it('stamps correlationId on the event envelope (no longer in the payload, #77)', async () => {
      const deps = makeDeps();
      const data = makeSliceData({ scenarios: [makeRawScenario()] });
      const handler = createAutomatedSliceHandler(data, deps as any);
      await handler(makeTriggerEvent({ correlationId: 'my-session' }));
      const evt = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0];
      expect(evt.correlationId).toBe('my-session');
      expect(evt.payload.correlationId).toBeUndefined();
    });
  });
  ...
  describe('Zero scenarios — pass-through', () => {
    it('publishes slice-level outcomes when no scenarios exist', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ 'customer-tier': 'gold' });
      const data = makeSliceData({
      ...
      expect(published.payload.f1).toBe('gold');
    });
    ...
    it('prefers session fact over defaultValue in slice-level outcomes', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ amount: '5000' });
      const data = makeSliceData({
        outcomes: [{
          name: 'pass-through',
      ...
      await handler(makeTriggerEvent());
      const payload = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0].payload;
      expect(payload.f1).toBe('5000');
    });

    it('resolves calculatedValue in slice-level outcome facts', async () => {
      const deps = makeDeps();
      deps.eventStore.getCorrelationFactValues.mockReturnValue({ orderValue: '200' });
      const data = makeSliceData({
        outcomes: [{
    ...
    it('merges triggering event payload into fact values for slice-level outcomes', async () => {
      ...
      const payload = (deps.eventBus.publish as ReturnType<typeof vi.fn>).mock.calls[0][0].payload;
      expect(payload.f1).toBe('platinum');
    });
    ...
    it('waits (does not publish) when slice has scenarios but none eligible', async () => {
      ...
      const deps = makeDeps();
      deps.eventStore.getCorrelationEventTypes.mockReturnValue(new Set<string>());
      const data = makeSliceData({
  ...
  describe('error handling', () => {
    it('catches and logs errors without rethrowing', async () => {
      const deps = makeDeps();
      // Force an unexpected error by making getCorrelationEventTypes throw
      deps.eventStore.getCorrelationEventTypes.mockImplementation(() => {
  ...
  describe('slice-tool-failed — surfaces tool errors regardless of match status', () => {
    ...
    it('a clean result that omits a RULE-REFERENCED output does NOT fail — left unset, surfaced as a stall (not slice-tool-failed)', async () => {
      // The tool succeeds but returns no key matching the declared returnedFact,
      // and a scenario rule compares that fact. Under the untrusted-output model
      // we must NOT fabricate a value (it would corrupt the comparison) and must
      // NOT raise slice-tool-failed (the tool didn't fail) — leave it unset so
      // the no-scenario-matched stall path reports it.
      mockEvaluateBusinessRules.mockResolvedValue(false);
      const deps = makeDeps();
      ...
      const publish = deps.eventBus.publish as any;
      // No slice-tool-failed (clean execution, not a tool failure).
      expect(publish.mock.calls.find((c: any[]) => c[0].type === 'slice-tool-failed')).toBeUndefined();
      // No outcome fired (rule referenced an unset fact → no match).
      expect(publish.mock.calls.find((c: any[]) => c[0].type === 'my-outcome')).toBeUndefined();
    });
...
describe('applyJobResultToFacts — invalid mapping (field not returned by tool) is a FATAL error', () => {
  const job = {
    toolId: 'find-json-record',
    name: 'customer-looked-up',
    // A stale `record` mapping (leftover from the old connector-outputParam
    // dialog) sits alongside the real field-level mappings. `record` is not a
    // key the tool returns — an incorrect output contract.
    resolvedOutputMappings: {
      id: { factName: 'id' },
      name: { factName: 'name' },
      tier: { factName: 'tier' },
      record: { factName: 'customer' },
    },
  } as any;

  it('still applies the valid fields but records the bad mapping as a FATAL error', () => {
    // find-json-record returns the flat record — no `record` key.
    const jobResult = { id: 'user-2', name: 'Jane Smith', tier: 'Member' };
    const allFactValues: Record<string, any> = {};
    const recordToolError = vi.fn();

    applyJobResultToFacts(jobResult, job, allFactValues, recordToolError, 'automation', 'customer-looked-up');

    // The valid fields are still extracted...
    expect(allFactValues.id).toBe('user-2');
    expect(allFactValues.name).toBe('Jane Smith');
    expect(allFactValues.tier).toBe('Member');
    // ...but the stale `record` mapping has no value to bind.
    expect(allFactValues.customer).toBeUndefined();

    // The invalid mapping is FATAL: the slice's output contract is wrong, so
    // the handler must fail the slice rather than emit a partial outcome. The
    // message names the offending field and the keys the tool actually returned.
    expect(recordToolError).toHaveBeenCalledTimes(1);
    const arg = recordToolError.mock.calls[0][0];
    expect(arg.fatal).toBe(true);
    expect(arg.error).toContain('record');
    expect(arg.error).toContain('id, name, tier');
  });

  it('still treats an { ok: false } connector envelope as fatal', () => {
    const recordToolError = vi.fn();
    applyJobResultToFacts(
      { ok: false, error: 'channel_not_found' },
      { ...job, resolvedOutputMappings: {} },
      {},
      recordToolError,
      'automation',
      'customer-looked-up',
    ...
    expect(recordToolError).toHaveBeenCalledTimes(1);
    expect(recordToolError.mock.calls[0][0].fatal).toBe(true);
  });
});
