...
describe('TodoStore', () => {
  ...
  describe('getBySession', () => {
    it('returns all todos for a workflow session', () => {
      store.upsert(makeTodo({ id: 'todo-1', workflowSessionId: 'sess-1' }));
      store.upsert(makeTodo({ id: 'todo-2', workflowSessionId: 'sess-1' }));
      store.upsert(makeTodo({ id: 'todo-3', workflowSessionId: 'sess-2' }));
      const result = store.getBySession('sess-1');
      expect(result.length).toBe(2);
    });
  });

  describe('findBySliceAndSession', () => {
    it('finds existing todo for a slice+session', () => {
      store.upsert(makeTodo({ id: 'todo-1', sliceName: 'approve', workflowSessionId: 'sess-1' }));
      const result = store.findBySliceAndSession('approve', 'sess-1');
      expect(result).not.toBeNull();
