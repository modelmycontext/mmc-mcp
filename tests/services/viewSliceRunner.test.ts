...
describe('executeViewSlice', () => {
    ...
    const slice = makeViewSlice({
      queries: [{
        ...
        jobLink: {
          ...
          outputMappings: { rows: { factId: 'f-rows', factName: 'rows' } },
        },
      }],
      facts: [{ id: 'f-rows', name: 'rows' }],
    });
      ...
      queries: [
        {
          ...
          jobLink: { job: { id: 'j1', name: 'rows', toolId: 'tool-a' }, outputMappings: { rows: { factId: 'f-rows', factName: 'rows' } } },
        },
        {
          id: 'q2', name: 'count-q', facts: [{ id: 'f-count', name: 'count' }],
          jobLink: { job: { id: 'j2', name: 'count', toolId: 'tool-b' }, outputMappings: { count: { factId: 'f-count', factName: 'count' } } },
        },
      ],
    });
      ...
      queries: [
        {
          ...
          jobLink: { job: { id: 'j1', name: 'broken', toolId: 'tool-a' }, outputMappings: { rows: { factId: 'f-rows', factName: 'rows' } } },
        },
        {
          id: 'q2', name: 'ok', facts: [{ id: 'f-ok', name: 'ok' }],
          jobLink: { job: { id: 'j2', name: 'ok', toolId: 'tool-b' }, outputMappings: { ok: { factId: 'f-ok', factName: 'ok' } } },
        },
      ],
    });
  ...
  it('only includes facts from the slice contract in the projection (caller args do not leak)', async () => {
    ...
    const slice = makeViewSlice({
      queries: [{
        id: 'q1', name: 'rows-q', facts: [{ id: 'f-rows', name: 'rows' }],
        jobLink: { job: { id: 'j1', name: 'rows', toolId: 'tool' }, outputMappings: { rows: { factId: 'f-rows', factName: 'rows' } } },
      }],
    });
