...
describe('resolveJobParams — Formula-mode expressions (builder ƒ Formula toggle)', () => {
  // A mapping value that is neither an @ref nor a known factId is a Formula-mode
  // expression authored via FactCalculationPicker. Before formula support these
  // fell through to '' (the legacy bare-factId branch), so e.g. a send-email
  // bodyText constant silently produced an empty param.
  const factLookup = new Map<string, string>([['fact-application-id', 'application-id']]);

  it('returns a constant string mapping verbatim (was empty before formula support)', () => {
    const params = resolveJobParams(
      { id: 'j', name: 'j', inputMappings: { subject: 'Your Driving Academy enrolment is approved' } },
      factLookup,
      { 'application-id': 'app-123' },
    );
    expect(params.subject).toBe('Your Driving Academy enrolment is approved');
  });

  it('expands {{fact}} templates embedded in a constant body', () => {
    const params = resolveJobParams(
      {
        id: 'j',
        name: 'j',
        inputMappings: {
          bodyText: 'Hi, your enrolment application {{application-id}} has been approved. Follow up information will arrive soon.',
        },
      },
      factLookup,
      { 'application-id': 'app-123' },
    );
    expect(params.bodyText).toBe(
      'Hi, your enrolment application app-123 has been approved. Follow up information will arrive soon.',
    );
  });

  it('evaluates an arithmetic formula over fact names', () => {
    const params = resolveJobParams(
      { id: 'j', name: 'j', inputMappings: { fee: 'orderValue * 0.1' } },
      new Map(),
      { 'order-value': '200' },
    );
    expect(params.fee).toBe('20');
  });

  it('still resolves @fact refs and bare factIds unchanged (regression)', () => {
    const params = resolveJobParams(
      { id: 'j', name: 'j', inputMappings: { a: '@application-id', b: 'fact-application-id' } },
      factLookup,
      { 'application-id': 'app-123' },
    );
    expect(params.a).toBe('app-123');
    expect(params.b).toBe('app-123');
  });
});
