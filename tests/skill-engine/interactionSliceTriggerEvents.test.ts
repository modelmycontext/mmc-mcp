...
describe('loadAutomatedSliceMap', () => {
  ...
  it('skips slices whose name is in interfaceSliceNames set', async () => {
    mockReaddir
      .mockResolvedValueOnce([makeDirEntry('activity')])
      .mockResolvedValueOnce([makeFileEntry('activity.json')]);
    mockReadFile.mockResolvedValue(JSON.stringify({
    ...
    const result = await loadAutomatedSliceMap('/skills', new Set(['request-discount']));
    expect(result.size).toBe(0);
  });
...
describe('validateSlice', () => {
  ...
  describe('view pattern', () => {
    ...
    it('flags view whose scenarios have a then', () => {
      const result = validateSlice({
        queries: [{ id: 'q' }],
        scenarios: [{ given: [{ name: 'data-asked' }], then: [{ name: 'shown', facts: [] }] }],
      });
      expect(result.errors.map(e => e.code)).toContain('VIEW_HAS_THEN');
    });
