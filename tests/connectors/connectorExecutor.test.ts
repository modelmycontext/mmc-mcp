...
describe('ConnectorExecutor find-json-record', () => {
  it('should call find-json-record correctly', async () => {
    const mockUsers = [
    ...
    const run = executor.createExecutor('find-json-record', { collection: 'users', find: '123', returns: 'user' });
    ...
    expect(mockEventBus.publish).toHaveBeenCalledWith(expect.objectContaining({
      type: 'TOOL_CALLED',
      payload: expect.objectContaining({ tool: 'find-json-record' })
    }));
  });
  ...
  it('should handle template in find parameter', async () => {
    ...
    const executor = new ConnectorExecutor();
    const run = executor.createExecutor('find-json-record', { collection: 'users', find: '{{userId}}', returns: 'user' });
  ...
  it('should handle mappings correctly', async () => {
    ...
    const executor = new ConnectorExecutor();
    const run = executor.createExecutor('find-json-record', {
...
describe('ConnectorExecutor — json-paginated-read', () => {
  it('returns paginated rows wrapped in a { records } envelope matching the connector\'s declared outputParams', async () => {
    const all = [
      { id: 1, name: 'a' },
      { id: 2, name: 'b' },
      { id: 3, name: 'c' },
    ...
    const ctx = makeCtx({
      dataSources: { json: { read: vi.fn().mockResolvedValue(all) } },
    });
    const executor = new ConnectorExecutor();
    const run = executor.createExecutor('json-paginated-read', {
      collection: 'orders',
      startRow: 1,
      rows: 2,
    });
    ...
    // Connector declares outputParams: [{ name: "records" }]. The runtime
    // shape callers receive must match — applyJobResultToFacts looks up
    // jobResult[outputField], so a bare-array return would silently drop
...
describe('ConnectorExecutor — deep path mappings', () => {
  it('assigns a deep dot-path result key (e.g. nested.key)', async () => {
    ...
    const executor = new ConnectorExecutor();
    // After find-json-record, finalResult.user = {id:'123', name:'John Doe'}
...
describe('ConnectorExecutor — error cases', () => {
  it('throws "Tool not found" when action is unknown with no connectors and no special case', async () => {
    const ctx = makeCtx();
    const executor = new ConnectorExecutor([]);
