...
- **[Node.js](https://nodejs.org) 22+** — development and test runtime (uses the
  built-in `node:sqlite`). Run the server with `pnpm start` / `pnpm dev` (both via
  `tsx`).
- **[Bun](https://bun.sh) 1.x** — *optional*; the deployed Fly image still runs the
  server under Bun. A runtime-detecting shim (`src/shims/bun-sqlite.ts`) picks
  `bun:sqlite` under Bun and `node:sqlite` under Node, so the same source runs on
  either runtime.
...
> Development and tests run under **Node** (`pnpm test` → `vitest run`); see
> [Testing](#testing). Bun is needed only if you run the server on the Bun
> production path.
...
SLACK_TEAM_ID=T...
# Optional, auth (recommended for any networked deployment):
ADMIN_API_KEY_HASH=...       # sha256 hex of the /admin/* bearer key
MCP_ACCESS_TOKEN_HASH=...    # sha256 hex of the /mcp bearer token; when unset
                             # /mcp is OPEN — acceptable for loopback dev only
```

Values written at runtime (e.g. by `POST /admin/rotate-key`) are persisted to
`data/runtime.env`, which is loaded on boot with precedence over `.env`. On a
volume-backed deployment (Fly) this keeps rotated keys across restarts.
...
`config/config.json` is the **seed**. Changes made at runtime via
`PUT /admin/config` are written to `data/config.json`, which takes precedence
on every subsequent read — on a volume-backed deployment this is what survives
a machine restart. Delete `data/config.json` to fall back to the seed.
...
- **`src/connectors/`** — `Connector` interface and `ConnectorExecutor`.
...
```bash
pnpm test          # runs the full vitest suite under Node
pnpm test:watch    # vitest in watch mode
```

The test runner is **Vitest under Node** (`vitest run`). The runtime-detecting
SQLite shim ([`src/shims/bun-sqlite.ts`](src/shims/bun-sqlite.ts)) resolves to
`node:sqlite` under Node and `bun:sqlite` under Bun, so the same code runs in
tests, dev, and both production runtimes — no separate test shim required.
...
├── connectors/         ConnectorExecutor (server-side glue)
