...
- `queries[]` — read jobs (typically `json-read` connector calls) that run before scenario evaluation. Their `outputMappings` results end up in the fact map.
...
The test suite (~481 tests, 26 files) runs as **Vitest under Node**. SQLite goes
through a runtime-detecting shim so the same source runs on both runtimes:

- `src/shims/bun-sqlite.ts` — resolves `node:sqlite` under Node (tests, dev) and
  `bun:sqlite` under Bun (the Fly production image). Source imports this shim
  instead of `bun:sqlite` directly.
- `vitest.config.ts` — declares the `@src`/`@sdk`/`@connectors` path aliases (and
  keeps a defensive `bun:sqlite` → shim alias for any stray import).
